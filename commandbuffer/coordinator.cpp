...
void read_dynamics_plugin( const char* filename ) {
    ...
    // locate the run function
    plugin.getcmd = (get_cmd_fun_t) dlsym(HANDLE, "get_command");
...
/**
  initially, both publish_state and get_command were separated out and explicitly
  invoked in the main coordinator loop.  However, it was determined that of these
  interfaces, it is only necessary to have explicit external access to publish_state
  for priming the controller and all other usages of both publish_state and get_command
  can be encapsulated by the run_dynamics mechanism on the dynamics plugin side.

  *This effectively means the DynamicsPlugin class does not need this function pointer*
*/
void get_command( ) {
    for( std::list< DynamicsPlugin >::iterator it = plugins.begin(); it != plugins.end(); it++ ) {
        (*it->getcmd)( );
