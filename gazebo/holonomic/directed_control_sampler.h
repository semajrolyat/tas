...
public:
  ...
  virtual unsigned int getBestControl( ompl::control::Control *control, const ompl::base::State *source, ompl::base::State *dest, const ompl::control::Control *previous ) {
    ...
    //       (I recommend putting from_state(.) in ship_state_c and pp_state_c)
    // generate state vector(s)
    ...
    // EMD: my work ends here for tonight
    // normalize the destination state quaternions
    double qnorm = 0.0;
    for( unsigned i = 3; i < 7; i++ )
      qnorm += q1[i] * q1[i];
    qnorm = std::sqrt( qnorm );
    ...
    // generate differential vector
    std::vector<double> dq( pp_state_c::size() );
