...
void init( int argc, char* argv[] ) {
  ...
  if( cpu_c::bind( coordinator_pid, cpu ) != cpu_c::ERROR_NONE ) {
    sprintf( errstr, "(coordinator.cpp) init() failed calling cpu_c::_bind(coordinator_pid,DEFAULT_CPU).\nExiting\n" );
  ...
  processor->runqueue.push_back( dynamics );
  processor->runqueue.push_back( prey );
  processor->runqueue.push_back( pred );
  std::make_heap(processor->runqueue.begin(),processor->runqueue.end(),compare_thread_p_t());

  prey->runqueue.push_back( prey_controller );
  std::make_heap(prey->runqueue.begin(),prey->runqueue.end(),compare_thread_p_t());

  pred->runqueue.push_back( pred_planner );
  pred->runqueue.push_back( pred_controller );
  std::make_heap(pred->runqueue.begin(),pred->runqueue.end(),compare_thread_p_t());
